...
A new policy `1cycle` is introduced, under this policy the model is trained for 1 cycle with less iterations than its predecessor linear_policy and instead, during the last iterations, the LR is decreased several orders of magnitude. In paper, it is demonstrated that following this policy training can reach optimal accuracy.
